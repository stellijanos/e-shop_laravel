            ...
            <div class="accordion-item">
                <h2 class="accordion-header">
                    <button class="accordion-button" type="button" data-bs-toggle="collapse"
                        data-bs-target="#panelsStayOpen-collapse-{{$index_1}}" aria-expanded="true"
                        aria-controls="panelsStayOpen-collapse-{{$index_1}}">
                        {{ucfirst($filter)}}
                    </button>
                </h2>
                <div id="panelsStayOpen-collapse-{{$index_1}}" class="accordion-collapse collapse show">
                    <div class="accordion-body">
                        @foreach($values as $key => $name) 
                            <div>
                                <input type="checkbox" class="mx-2 form-check-input" name="{{$filter}}[]"
                                    id="{{$key}}-{{$index_1}}-{{$loop->index}}" value="{{$key}}" {{ in_array($key, $checked_categories) ? 'checked' : ''}} onchange="this.form.submit()">
                                <label for="{{$key}}-{{$index_1}}-{{$loop->index}}">{{$name}}</label>
                            </div>
                        @endforeach
                    </div>
                </div>
