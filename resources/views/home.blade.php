    ...
    <div class="row justify-content-flex-start">
        @foreach ($products as $product )
            <div class="bg-body-secondary text-light-emphasis rounded text-center shadow pt-3" style="width:150px">
                ...
                <figcaption>
                    <p class="text-truncate fs-3 fw-bold" style="max-width: 150px;">{{$product->name}}</p>
                    <p class="text-truncate" style="max-width: 150px;">{{$product->description}}</p>
                    <p class="fs-5 text-start fw-bold">${{$product->price}}</p>
